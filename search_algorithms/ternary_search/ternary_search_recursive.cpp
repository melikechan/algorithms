...
int ternarySearchRecursive (int* arr, int l, int r, int elem) {
    ...
    else if (arr[lowMid] > elem) return ternarySearchRecursive(arr, l, lowMid - 1, elem); 
      //set right to 1/3 - 1
      //because we looked at 1/3 but if it's smaller than 1/3, it has to be at 1st subarray
    else if (elem > arr[highMid]) return ternarySearchRecursive(arr, highMid + 1, r, elem);
      //set left to 2/3 + 1
      //also we looked at 2/3 but if it's greater than 2/3, it has to be at 3rd subarray
    else return ternarySearchRecursive(arr, lowMid + 1, highMid - 1, elem);
      //set left to 1/3 + 1
      //set right to 2/3 - 1
...
int main(void) {
  ...
  ind = ternarySearchRecursive(arr, 0, size - 1, element);
